...
public struct PopupOptions {
    ...
    public init(shouldDismissOnTapOutside: Bool = true, overlayColor: UIColor = UIColor.init(white: 0, alpha: 0.7)) {
...
extension INavigationService {
    ...
    func startPush(to page: UIViewController, options: PushOptions) {
        ...
        case .popup(let option):
            let presenterPage = PresenterController(contentPage: page, options: option)
